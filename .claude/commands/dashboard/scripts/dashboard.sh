...
calculate_priority() {
  ...
  # Deprioritize routine automation PRs
  if echo "$labels" | grep -qE "automation/merge|automation/tfgen-provider-docs"; then
    priority=$((priority - 30))
  fi
...
if [ -f "$TMPDIR/workflows.json" ]; then
  PUSH_RUN=$(cat "$TMPDIR/workflows.json" | jq -r '[.[] | select(.name == "push" or .name == "Push")] | .[0] // empty')
  if [ -n "$PUSH_RUN" ]; then
    LAST_PUSH_CONCLUSION=$(echo "$PUSH_RUN" | jq -r '.conclusion // "in_progress"')
    LAST_PUSH_TIME=$(echo "$PUSH_RUN" | jq -r '.createdAt // "unknown"')
  fi
fi
...
jq -n \
  ...
  --arg last_push_conclusion "$LAST_PUSH_CONCLUSION" \
  --arg last_push_time "$LAST_PUSH_TIME" \
  ...
  '{
    ...
    deployment: {
      last_push_conclusion: $last_push_conclusion,
