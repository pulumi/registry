---
description: Review registry changes for quality before committing (checks code, YAML, CI, and infrastructure).
---

# Registry Review Command

**Use this when:** You're working on registry changes and want to check quality before committing, or when you want feedback without the full PR approval workflow.

---

## Usage

`/registry-review [PR_NUMBER]`

Reviews `pulumi/registry` changes for code quality, YAML validity, CI correctness, and infrastructure safety.
...
## Instructions for Registry Review
...
1. Start by running `gh pr view <PR_NUMBER> --json title,body,files,additions,deletions,labels` to get PR metadata
...
Format your review comment in well-structured markdown for readability (headings, bullet lists, code blocks, etc.).
...
**Quick reference**: Check Go code quality, CI workflow correctness, provider YAML validity, build system compatibility, infrastructure safety, BUILD-AND-DEPLOY.md currency, community package list integrity, and general spelling/grammar/links.
