...
- Cross-page links must drop the `.md`. A relative link between the provider's registry pages (for example `_index.md` linking to the install page) must target `./installation-configuration`, not `./installation-configuration.md`. **Warn** on a relative markdown link whose target ends in `.md`: `]\((?!https?://)[^)]*\.md\)`. Flagged in review of [#11661](https://github.com/pulumi/registry/pull/11661).
- Published-SDK entries should be markdown links, not bare URLs: link the package name to its package page (`[`<pkg>`](<url>)`) rather than printing the URL on its own line. **Warn** on a bare URL under an SDK entry. Flagged in review of [#11661](https://github.com/pulumi/registry/pull/11661).
