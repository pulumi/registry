...
module.exports = {
    ...
    getKeywords(page) {
        return (page.params?.search?.keywords || []).concat(
            rank.getImplicitKeywords(page),
        );
    },
        ...
        [2].forEach((level) => {
            ...
            headings.forEach((h) => {
                ...
                subheads.push({
                    ...
                    content:
                        h.nextElementSibling?.nodeName === "P"
    ...
    isBoosted(page) {
        return (
            rank.isImplicitlyBoosted(page) ||
            rank.isExplicitlyBoosted(page) ||
            undefined
        );
    },
        ...
        return (
            hugoPageItems
                ...
                .filter((item) => {
                    ...
                    const isBlockedFromExternalSearch =
                        item.block_external_search_index === true;
                    const isMissingObjectID = item.objectID === "";
                    const isZeroRanked = this.getRank(item) === 0;
                    const isRedirect =
                        item.params.redirect_to &&
                        item.params.redirect_to !== "";

                    if (
                        isDraft ||
                        isBlockedFromExternalSearch ||
                        isRedirect ||
                        isMissingObjectID ||
                        isZeroRanked
                    ) {
                ...
                if (domContent.subheads) {
                    domContent.subheads.forEach((subhead) => {
                        ...
                        secondaries.push({
                            ...
                            ancestors: this.makeAncestorsList([
                                ...ancestors,
                                pageObject.title,
                            ]),
                            title: subhead.title,
