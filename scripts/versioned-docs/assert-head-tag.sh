...
#
# inject-version-switcher.sh inserts noindex/canonical/loader tags before the
# first </head> (falling back to </body>). This assertion keeps that invariant
# honest: every page in a snapshot must offer an injection point. It fails if
# any page has neither </head> nor </body>, and reports how many pages lack
# </head> so a silent template/minifier change is visible in CI.
#
# Usage: assert-head-tag.sh <snapshot-dir>
#
set -euo pipefail
