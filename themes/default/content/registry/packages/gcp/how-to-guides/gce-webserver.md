...
1. [Configure GCP credentials](/registry/packages/gcp/installation-configuration/)
...
1. Open `index.js` and replace the contents with the following:
    ...
    This example uses the [@pulumi/gcp](/registry/packages/gcp) package to create and manage three Google Cloud resources: a [gcp.compute.Network](/registry/packages/gcp/api-docs/compute/network/) in which the virtual machine will run, a [gcp.compute.Firewall](/registry/packages/gcp/api-docs/compute/firewall/) which allows access for incoming SSH access, and a [gcp.compute.Instance](/registry/packages/gcp/api-docs/compute/instance/) which is created inside the network from the Debian 9 base image.
...
1. To view the provisioned resources on the command line, run `pulumi stack`. You'll also see two [stack outputs](/docs/intro/concepts/stack#output) corresponding to the IP and full-qualified host name of the virtual machine instance we've created.
...
1. We can use `pulumi stack output` to get the value of stack outputs from the CLI.  So we can `curl` the virtual machine instance to see the HTTP server running there. Stack outputs can also be viewed ine the Pulumi Cloud console.
