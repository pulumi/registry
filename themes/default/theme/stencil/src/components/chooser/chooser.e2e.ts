...
describe("pulumi-chooser", () => {
    ...
    it("renders a language chooser as a toolbar with a logo label and abbreviated tabs", async () => {
        const page = await newE2EPage();
        // pulumi-root initializes the store, which supplies the chooser's selection
        // (and with it the toolbar label's logo + name), just like on a real page.
        await page.setContent(
            `<pulumi-root></pulumi-root><pulumi-chooser type="language" options="typescript,go,python,csharp"></pulumi-chooser>`,
        );
