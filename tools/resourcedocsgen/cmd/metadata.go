...
// defaultAnyTFPublisher is the publisher applied to providers generated by the AnyTF
// pipeline (via `metadata from-urls`) when the pipeline hasn't stamped a publisher
// into the schema. See https://github.com/pulumi/registry/issues/12073.
const defaultAnyTFPublisher = "Pulumi"
...
func packageMetadataFromURLsCmd(client HTTPDoer, metadataDir, packageDocsDir *string) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
		...
		// Providers reaching us via `from-urls` are the ones generated by the AnyTF
		// pipeline (see https://github.com/pulumi/registry/issues/12073). These are
		// Pulumi-generated and Pulumi-supported, so "Pulumi" is the default publisher
		// unless the pipeline has stamped a specific publisher into the schema (for
		// example, to keep a provider under its upstream name during a deprecation
		// period, or to mark it as an experimental "Pulumi Labs" project).
		if mainSpec.Publisher == "" {
			mainSpec.Publisher = defaultAnyTFPublisher
		}
