...
// PackageTreeItem is a type for representing a package in a navigable tree format starting from the
// top-level/index/root of a package.
...
func generatePackageTree(rootMod modContext) ([]PackageTreeItem, error) {
	...
	conflictResolver := rootMod.context.newModuleConflictResolver()
